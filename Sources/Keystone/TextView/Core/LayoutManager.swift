...
final class LayoutManager {
    ...
    /// Code folding manager for tracking foldable regions
    var codeFoldingManager: CodeFoldingManager?
    /// Called when a fold toggle is tapped, with the line number (0-indexed)
    ...
    var textContainerInset: RSLayoutEdgeInsets = .zero
    var safeAreaInsets: RSLayoutEdgeInsets = .zero
    ...
    // MARK: - Views
    let gutterContainerView = RSLayoutView()
    ...
    private var foldIndicatorViewReuseQueue = ViewReuseQueue<DocumentLineNodeID, FoldIndicatorView>()
    private var visibleLineIDs: Set<DocumentLineNodeID> = []
    private let linesContainerView = RSLayoutView()
    private let gutterBackgroundView = GutterBackgroundView()
    private let lineNumbersContainerView = RSLayoutView()
    private let foldIndicatorsContainerView = RSLayoutView()
    private let gutterSelectionBackgroundView = RSLayoutView()
    private let lineSelectionBackgroundView = RSLayoutView()
    ...
    init(lineManager: LineManager,
        ...
        self.gutterContainerView.isUserInteractionEnabled = true // Allow interaction for fold buttons
...
extension LayoutManager {
    ...
    private func layoutGutter() {
        ...
        foldIndicatorsContainerView.frame = CGRect(x: 0, y: 0, width: totalGutterWidth, height: contentSize.height)
    }
    ...
    private func layoutLinesInViewport() {
        ...
        // Get hidden lines from code folding manager only if code folding is enabled
        let hiddenLines: Set<Int> = (showCodeFolding && codeFoldingManager != nil) ? codeFoldingManager!.hiddenLines : []
        ...
        while let line = nextLine, maxY < insetViewport.maxY, constrainingLineWidth > 0 {
            let isLineHidden = showCodeFolding && hiddenLines.contains(line.index)

            if isLineHidden {
                // Hidden line: set height to 0 and skip rendering
                contentSizeService.setSize(of: line, to: CGSize(width: 0, height: 0))
                // Enqueue any existing views for this hidden line
                if lineNumberLabelReuseQueue.visibleViews[line.id] != nil {
                    lineNumberLabelReuseQueue.enqueueViews(withKeys: [line.id])
                }
                if foldIndicatorViewReuseQueue.visibleViews[line.id] != nil {
                    foldIndicatorViewReuseQueue.enqueueViews(withKeys: [line.id])
                }
                // Line fragment views will be cleaned up by the disappeared fragments logic below
            } else {
                // Visible line: process normally
                ...
                layoutFoldIndicatorView(for: line)
    ...
    private func layoutFoldIndicatorView(for line: DocumentLineNode) {
        guard showCodeFolding, let codeFoldingManager = codeFoldingManager else {
            return
        }
        // Check if this line has a foldable region starting here
        guard let region = codeFoldingManager.regionStarting(atLine: line.index) else {
            // No fold region starts at this line, enqueue any existing view
            if foldIndicatorViewReuseQueue.visibleViews[line.id] != nil {
                foldIndicatorViewReuseQueue.enqueueViews(withKeys: [line.id])
            }
            return
        ...
        let foldIndicatorView = foldIndicatorViewReuseQueue.dequeueView(forKey: line.id)
        if foldIndicatorView.superview == nil {
            foldIndicatorsContainerView.addSubview(foldIndicatorView)
        }

        let lineController = lineControllerStorage.getOrCreateLineController(for: line)
        let fontLineHeight = theme.lineNumberFont.lineHeight
        let indicatorSize: CGFloat = 14
        let xPosition = safeAreaInsets.left + gutterWidthService.gutterLeadingPadding + gutterWidthService.lineNumberWidth + 1
        var yPosition = textContainerInset.top + line.yPosition
        if lineController.numberOfLineFragments > 1 {
            yPosition += (fontLineHeight * lineHeightMultiplier - indicatorSize) / 2
        } else {
            yPosition += (lineController.lineHeight - indicatorSize) / 2
        }

        let isFolded = codeFoldingManager.foldedRegionIds.contains(region.id)
        foldIndicatorView.isFolded = isFolded
        // Use blue color for folded indicators to draw attention, otherwise use theme color
        foldIndicatorView.indicatorColor = isFolded ? .systemBlue : theme.lineNumberColor
        foldIndicatorView.frame = CGRect(x: xPosition, y: yPosition, width: indicatorSize, height: indicatorSize)

        let lineIndex = line.index
        foldIndicatorView.onTap = { [weak self] in
            self?.onFoldToggle?(lineIndex)
        }
    }
    ...
    private func setupViewHierarchy() {
        ...
        foldIndicatorsContainerView.removeFromSuperview()
        ...
        let allFoldIndicatorKeys = foldIndicatorViewReuseQueue.visibleViews.keys
        foldIndicatorViewReuseQueue.enqueueViews(withKeys: Set(allFoldIndicatorKeys))
        ...
        gutterContainerView.addSubview(foldIndicatorsContainerView)
    }

    private func updateShownViews() {
        let selectedLength = selectedRange?.length ?? 0
        let showGutter = showLineNumbers || showCodeFolding
        gutterBackgroundView.isHidden = !showGutter
        lineNumbersContainerView.isHidden = !showLineNumbers
        foldIndicatorsContainerView.isHidden = !showCodeFolding
        gutterSelectionBackgroundView.isHidden = !lineSelectionDisplayType.shouldShowLineSelection || !showGutter || !isEditing
