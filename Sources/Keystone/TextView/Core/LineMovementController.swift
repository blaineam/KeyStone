...
/// Cross-platform text layout direction for macOS
enum RSTextLayoutDirection: Int {
    case left = 0
    case right = 1
    case up = 2
    case down = 3
...
final class LineMovementController {
    ...
    func location(from location: Int, in direction: RSTextLayoutDirection, offset: Int) -> Int? {
        let newLocation: Int?
        switch direction {
        case .left:
            newLocation = locationForMoving(fromLocation: location, by: offset * -1)
