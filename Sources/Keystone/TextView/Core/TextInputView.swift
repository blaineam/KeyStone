...
final class TextInputView: UIView, UITextInput {
    ...
    /// Set to true during text updates to prevent tokenizer from querying RedBlackTree before it's ready
    var isUpdatingText: Bool {
        get { customTokenizer.isUpdatingText }
        set { customTokenizer.isUpdatingText = newValue }
    }
    ...
    /// Use this instead of setting `string` directly when you want to sync text without clearing undo history.
    ...
    /// Forces an immediate layout refresh. Use after fold state changes to update the view.
    ...
    override func layoutSubviews() {
        ...
        if notifyInputDelegateAboutSelectionChangeInLayoutSubviews {
            // Set isUpdatingText to prevent tokenizer crashes when keyboard queries line boundaries
            let wasUpdating = isUpdatingText
            isUpdatingText = true
            inputDelegate?.selectionWillChange(self)
            inputDelegate?.selectionDidChange(self)
            isUpdatingText = wasUpdating
        }
...
extension TextInputView {
    ...
    private func setStringWithUndoAction(_ newString: NSString) {
        ...
        timedUndoManager.registerUndo(withTarget: self) { textInputView in
            // Set isUpdatingText to prevent tokenizer crashes when keyboard queries line boundaries
            textInputView.isUpdatingText = true
            defer { textInputView.isUpdatingText = false }
