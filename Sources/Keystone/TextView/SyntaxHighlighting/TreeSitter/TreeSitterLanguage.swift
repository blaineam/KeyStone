...
private extension TreeSitterInternalLanguage {
    ...
    private static func makeInternalQuery(from query: TreeSitterLanguage.Query?, with language: OpaquePointer) -> TreeSitterQuery? {
        if let string = query?.string {
            do {
                let result = try TreeSitterQuery(source: string, language: language)
                #if DEBUG
                print("[TreeSitter] Successfully parsed query (\(string.count) chars)")
                #endif
                return result
            } catch {
                #if DEBUG
                print("[TreeSitter] Invalid TreeSitterLanguage.Query. Error: \(error). Query:\n\(string)")
                #endif
                return nil
            }
        } else {
            #if DEBUG
            print("[TreeSitter] No query string provided")
            #endif
            return nil
        }
    }
