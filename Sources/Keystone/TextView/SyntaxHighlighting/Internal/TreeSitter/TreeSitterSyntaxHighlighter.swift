#if canImport(UIKit)
import UIKit
private typealias RSFont = UIFont
private typealias RSFontDescriptor = UIFontDescriptor
private let RSTraitBold = UIFontDescriptor.SymbolicTraits.traitBold
private let RSTraitItalic = UIFontDescriptor.SymbolicTraits.traitItalic
#elseif canImport(AppKit)
import AppKit
private typealias RSFont = NSFont
private typealias RSFontDescriptor = NSFontDescriptor
private let RSTraitBold = NSFontDescriptor.SymbolicTraits.bold
private let RSTraitItalic = NSFontDescriptor.SymbolicTraits.italic
#endif
...
private extension TreeSitterSyntaxHighlighter {
    private func setAttributes(for tokens: [TreeSitterSyntaxHighlightToken], on attributedString: NSMutableAttributedString) {
        ...
        for token in tokens {
            ...
            var symbolicTraits: RSFontDescriptor.SymbolicTraits = []
            if let isBold = attributedString.attribute(.isBold, at: token.range.location, effectiveRange: nil) as? Bool, isBold {
                symbolicTraits.insert(RSTraitBold)
            }
            if let isItalic = attributedString.attribute(.isItalic, at: token.range.location, effectiveRange: nil) as? Bool, isItalic {
                symbolicTraits.insert(RSTraitItalic)
            }
            let currentFont = attributedString.attribute(.font, at: token.range.location, effectiveRange: nil) as? RSFont
